...
LLM ChatBot Model Fine-tuning on Vietnamese Dataset

This code fine-tunes the LLM (Language Model) using the LoRa Alpaca pipeline.

Pipeline Overview:
1. Translate the existing English-instructed dataset to a Vietnamese dataset (translate_dataset_....py).
2. LORA: Finetuned Model -> Further Compression to 8-Bit or 4-Bit Quantization -> Further Distillation to a Smaller Model.
3. Building an application for the ChatBot conversation.

List of Inference Language Models:
- NousResearch: 4-bit Quantization
...
def train(
    # model/data params
    base_model: str = "bigscience/bloomz-560m", #"facebook/opt-350m",  # the only required argument
    ...
    data_path: str = "/media/rick/f7a9be3d-25cd-45d6-b503-7cb8bd32dbd5/Instruction_finetune_dataset/Belle_open_source_1M.json", #"/home/rick/Integrated_APP/Multimodal_Integrated_App/Language/data/alpaca_52k_instruction_cleaned.json",
    output_dir: str = "./lora-alpaca_BlOOMZ_560m_1M_CN/",
    ...
    wandb_run_name: str = "Instruction_Finetune_BLOOM_560m_LORA_1M_CN",
